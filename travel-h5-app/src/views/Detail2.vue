...
.day-tabs {
  display: flex;
  flex-wrap: nowrap;          /* 🚨 补丁：坚决不换行，一硬到底 */
  overflow-x: auto;           /* 🚨 补丁：当元素超出屏幕时，允许横向滚动 */
  -webkit-overflow-scrolling: touch; /* 🚨 补丁：让手机端手指划动有原生丝滑感 */
  padding: 12px 16px; 
  
  background-color: #f7f8fa;  /* 🚨 将背景色改成和页面大背景一致的浅灰色 */
  gap: 10px;                  /* 保持每个 tab 之间的物理间距 */                /* 保持每个 tab 之间的物理间距 */
}
.day-tabs::-webkit-scrollbar {
  display: none;
}
.day-tab-item {
  flex-shrink: 0;             /* 🚨 核心：绝对不允许手机去压缩我的宽度 */
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 4px;
  min-width: 90px;            /* 🚨 给一个固定的最小宽度，保证第X天能完美显示 */
  padding: 8px 14px;
  border-radius: 20px;
...
.transport-boxes {
  display: flex;
  flex-direction: column;     /* 🚨 绝招：在手机端，改为上下两层堆叠，腾出巨大空间 */
  gap: 12px;                  /* 两个卡片之间的间距 */
  margin-top: 10px;
}
...
.price-unit { font-size: 12px; color: #969799; }

/* ✈️ 机票专属布局：让近三天的价格横向平铺开 */
.flight-days {
  display: flex;
  justify-content: space-between; /* 撑满卡片宽度，三份平分 */
  gap: 8px;
}

.flight-day {
  flex: 1;                    /* 三个日期均分横向空间 */
  background: #ffffff;         /* 用纯白内衬托起价格，更精致 */
  padding: 8px 4px;
  border-radius: 6px;
  text-align: center;
